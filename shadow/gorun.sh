...
if [ $# -ne 14 ]; then
    ...
    echo "$0 1 1000 15000 1 10 50 130 60 160 5 0.0 4 4000 7" 
    exit 1
fi
...
"$PYTHON" topogen.py $nodes $min_bandwidth $max_bandwidth $min_latency $max_latency $steps $pkt_loss $msg_size $num_frag $num_publishers $publisher_id $message_delay $D_announce
